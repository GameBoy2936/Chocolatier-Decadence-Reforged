...
-- Log the encounter with the global catalogue
if char then
...
DebugOut("UI", string.format("Rendering Character Dialog for: %s", char and char.name or "Unknown"))

-- Read custom button properties from the parent engine call
...
if char then displayHappiness = char:GetHappiness() end

if gDialogTable.mood == "angry" then
	displayHappiness = Character.kAngry - 10
elseif gDialogTable.mood == "happy" then
	displayHappiness = Character.kHappy + 10
elseif gDialogTable.mood == "neutral" then
	displayHappiness = Character.kNeutral
end
...
-- Calculate properties for the single centered dismissal button
...
MakeDialog
{
	Window
	{
		...
		CharWindow { x = 49, y = 0, name = char.name, happiness = displayHappiness },
	}
}

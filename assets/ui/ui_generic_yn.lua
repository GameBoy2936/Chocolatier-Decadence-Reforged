...
local yesLength = gDialogTable.yes_length or "medium"
local noLength = gDialogTable.no_length or "medium"
...
-- ----------------------------------------------------------------------------
-- We must translate the localization key (e.g., "confirm_quit") into readable text
-- BEFORE we calculate its pixel length or wrap it in HTML font tags.

if string.sub(text, 1, 1) == "#" then
	-- Strip the hash (legacy indicator for "already translated text")
...
DebugOut("UI", string.format("Rendering Generic Yes/No Prompt. Message length: %d characters.", string.len(text)))

--[[---------------------------------------------------------------------------
	Chocolatier Three: Decadence by Design Reforged (Building Selector)
...
-- Hardcoded UI hex colors to categorize buildings based on their internal engine types
local buildingTypeStyles = {
	...
	-- Flag used to denote unstaffed/empty wanderer buildings
...
for portName, port in pairs(_AllPorts) do
	-- Ignore locked ports and virtual system buildings
...
for _, building in ipairs(buildings) do
	...
	table.insert(items, Button { 
		...
		label = label, 
		command = function() 
			DebugOut("DEV", string.format("Admin Selection: Chose building '%s'.", tempBuilding.name))
...
MakeDialog
{
	...
	BSGWindow { 
		x = gDialogTable.x, y = gDialogTable.y, w = w * 4, h = 600, fit = true, color = { 1, 1, 1, 0.9 }, SetStyle(devMenuStyle),
